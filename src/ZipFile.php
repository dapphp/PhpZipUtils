...
require_once 'ZipDirectoryEntry.php';
require_once 'ZipCentralDirectory.php';
require_once 'ZipFileIterator.php';

use Dapphp\PhpZipUtils\ZipFileEntry;
use Dapphp\PhpZipUtils\ZipDirectoryEntry;
use Dapphp\PhpZipUtils\ZipCentralDirectory;
use Dapphp\PhpZipUtils\ZipFileIterator;
use Dapphp\PhpZipUtils\ZCrypt;
...
class ZipFile
{
    ...
    // Flag constants - from PHP ZipArchive class
    const FL_NOCASE = 1;
    const FL_NODIR = 2;
     ...
     * Example:
     *
     *     $zip = new \Dapphp\PhpZipUtils\ZipFile();
    ...
    {
        if ($this->_centralDirectory instanceof ZipCentralDirectory) {
    ...
    public function open($zipfile, $flags = 0)
    {
        ...
        if ($exists) {
            ...
            if (is_dir($zipfile)) {
                $this->_setError(self::ER_NOZIP, 'Zip file is a directory');
                return false;
            }
        }
     ...
     *
     * @return ZipFileIterator The file iterator
     */
    public function getFiles()
    {
        return new ZipFileIterator($this, $this->getCentralDirectory());
    }
    ...
    public function addEmptyDir($dirname)
    {
        ...
        if ( ($i = $this->locateName($dirname)) >= 0) {
            ...
            $this->_files[$i]->state = ZipDirectoryEntry::ZIP_ST_RENAMED;
        } else {
            $file = new ZipDirectoryEntry();
            $file->filename = $dirname;
            $file->filenameLength = strlen($dirname);
            $file->state = ZipDirectoryEntry::ZIP_ST_ADDED;
            $this->_files[] = $file;
        }
    ...
    public function deleteName($name)
    {
        ...
        foreach($this->_files as $idx => $file) {
            if ($dir && strpos($file->filename, $name) === 0) {
                $this->_files[$idx]->state = ZipDirectoryEntry::ZIP_ST_DELETED;
                $return = true;
    ...
    public function deleteIndex($index)
    {
        ...
        $this->_files[$index]->state = ZipDirectoryEntry::ZIP_ST_DELETED;
        $this->_changed = true;
    ...
    /**
     * Return the entry contents from its name
     *
     * @param string $name Name of the entry
     * @param int $length The length to be read, if 0 then read the entire entry
     * @param int $flags Flags to use for finding the file (ZipFile::FL_NOCASE)
     */
    public function getFromName($name, $length = 0, $flags = 0)
    {
        ...
        $entry = $this->getFromIndex($index);

        if ((int)$length > 0) {
            return substr($entry->data, 0, $length);
        } else {
            return $entry->data;
        }
    }
     ...
     * @param int $flags One or moere flags (ZipFile::FL_NOCASE, ZipFile:;FL_NODIR)
     * @return int -1 if not found, otherwise the numeric index of the file
     */
    public function locateName($name, $flags = 0)
    {
        $nocase = false;
        if (($flags & self::FL_NOCASE) > 0) {
            $nocase = true;
            $name   = strtolower($name);
        }

        for ($i = 0; $i < sizeof($this->_files); ++$i) {
            /* @var $entry ZipDirectoryEntry */
            $entry = $this->_files[$i];
            if ($entry->state == ZipDirectoryEntry::ZIP_ST_DELETED) continue;

            if (($flags & self::FL_NODIR) > 0 && $entry->is_directory == true) continue;

            if ($nocase) {
                $ent = strtolower($entry->filename);
            } else {
                $ent = $entry->filename;
            }
    ...
    public function setCommentIndex($index, $comment)
    {
        ...
        if (isset($this->_files[$index]) && ZipDirectoryEntry::ZIP_ST_DELETED != $this->_files[$index]->state) {
    ...
    private function _init()
    {
        ...
        $this->_readOnly           = true;
        $this->_files              = array();
        $this->_centralDirectory   = new ZipCentralDirectory();
        $this->_changed            = false;
    ...
    private function readFileData(ZipDirectoryEntry $cd, ZipDirectoryEntry $file)
    {
        if ($file->csize == 0) {
            $file->data = '';
            return true;
        ...
        if ($file->isEncrypted()) {
            ...
            if ($file->compressionMethod == 99) {
                // TODO: implement zip AES encryption
                return $this->_setError(self::ER_ENCRYPTED, 'File is AES encrypted, but AES is not yet supported');
            }
    ...
    private function decryptFileData(ZipDirectoryEntry $e)
    {
        require_once 'ZCrypt.php';

        $keys = array();

        ZCrypt::initEncryptionKeys($keys, $this->getArchivePassword());
        $encryptionHeader = substr($e->data, 0, self::ENCHEADERLEN);
        $e->data          = substr($e->data, self::ENCHEADERLEN);
        $decryptedHeader  = array();

        for ($i = 0; $i < self::ENCHEADERLEN; $i++) {
            $c = ord($encryptionHeader{$i});
            ZCrypt::zDecode($keys, $c);
            $decryptedHeader[$i] = $c;
        }

        $passwordCheck = $decryptedHeader[self::ENCHEADERLEN - 1];
        $actualCheck   = (($e->bitFlags & self::ZIP_GPBF_DATA_DESCRIPTOR) > 0) ?
                         (($e->time >> 8) & 0xff) :
                         (($e->crc >> 24) & 0xff);

        if ($passwordCheck != $actualCheck) {
            return $this->_setError(self::ER_BADPASS, 'Incorrect archive password');
        }

        $e->csize -= self::ENCHEADERLEN; // decrease compressed size by encryption header
    ...
    private function encryptFileData(&$data, ZipDirectoryEntry $e)
    {
        // init keys
        $temp = 0;
        $keys = array();
        ...
        for ($i = 0; $i < self::ENCHEADERLEN - 2; ++$i) {
            $c = (mt_rand(0, 0x7fffffff) >> 7) & 0xff;
            ZCrypt::zEncode($keys, $c, $temp);
            $encryptionHeader .= chr($c);
        }

        // calculate password check
        $c = ($e->crc >> 16) & 0xff;
        ZCrypt::zEncode($keys, $c, $temp);
        $encryptionHeader .= chr($c);
        ...
        $data = $encryptionHeader . $data;
    }

    private function decompressData(ZipDirectoryEntry $e)
    {
    ...
    private function readCentralDirectory($bufOffset, $buf, $eocd)
    {
        ...
        $tmp = unpack('v', substr($buf, $cdp, 2));
        $i   = array_shift($tmp);
        $cdp += 2;

        $tmp = unpack('v', substr($buf, $cdp, 2));
        $cd->setNentry(array_shift($tmp)); // number of cdir entries
        $cdp += 2;

        $tmp = unpack('V', substr($buf, $cdp, 4));
        $cd->setSize(array_shift($tmp));
        $cdp += 4;

        $tmp = unpack('V', substr($buf, $cdp, 4));
        $cd->setOffset(array_shift($tmp));
        $cdp += 4;

        $cd->setComment(null);
        $tmp = unpack('v', substr($buf, $cdp, 2));
        $comlen2 = array_shift($tmp);
        $cdp += 2;
    ...
    private function readZipDirectoryEntry(&$left, $local)
    {
        ...
        if (!$local) {
            $tmp = unpack('v', substr($cur, 0, 2));
            $de->versionMadeBy = array_shift($tmp);
            $cur = substr($cur, 2);
        } else {
            $de->versionMadeBy = 0;
        }
        ...
        $tmp = unpack('v', substr($cur, 0, 2));
        $de->versionNeeded = array_shift($tmp);
        $cur = substr($cur, 2);

        $tmp = unpack('v', substr($cur, 0, 2));
        $de->bitFlags = array_shift($tmp);
        $cur = substr($cur, 2);

        $tmp = unpack('v', substr($cur, 0, 2));
        $de->compressionMethod = array_shift($tmp);
        $cur = substr($cur, 2);

        $tmp = unpack('v', substr($cur, 0, 2));
        $dostime = array_shift($tmp);
        $cur = substr($cur, 2);

        $tmp = unpack('v', substr($cur, 0, 2));
        $dosdate = array_shift($tmp);
        $cur = substr($cur, 2);
        ...
        $tmp = unpack('V', substr($cur, 0, 4));
        $de->crc = array_shift($tmp);
        $cur = substr($cur, 4);

        $tmp = unpack('V', substr($cur, 0, 4));
        $de->csize = array_shift($tmp);
        $cur = substr($cur, 4);

        $tmp = unpack('V', substr($cur, 0, 4));
        $de->size = array_shift($tmp);
        $cur = substr($cur, 4);

        $tmp = unpack('v', substr($cur, 0, 2));
        $de->filenameLength = array_shift($tmp);
        $cur = substr($cur, 2);

        $tmp = unpack('v', substr($cur, 0, 2));
        $de->extraFieldLength = array_shift($tmp);
        $cur = substr($cur, 2);
        ...
        if ($local) {
            ...
            $tmp = unpack('v', substr($cur, 0, 2));
            $de->commentLength = array_shift($tmp);

            $tmp = unpack('v', substr($cur, 2, 2));
            $de->diskNumber = array_shift($tmp);

            $tmp = unpack('v', substr($cur, 4, 2));
            $de->internalAttributes = array_shift($tmp);

            $tmp = unpack('V', substr($cur, 6, 4));
            $de->externalAttributes = array_shift($tmp);

            $tmp = unpack('V', substr($cur, 10, 4));
            $de->offset = array_shift($tmp);
    ...
    private function zipCheckConsistency(ZipCentralDirectory $cd)
    {
        if ($cd->getNentry()) {
        ...
        for ($i = 0; $i < $cd->getNentry(); $i++) {
            ...
            $o = 0;
            if (($temp = $this->readZipDirectoryEntry($o, true)) === false) {
                return false;
            }
    ...
    private function readDataDescriptor(ZipDirectoryEntry $cde, ZipDirectoryEntry &$local)
    {
    ...
    private function zipHeaderCompare(ZipDirectoryEntry $h1, $local1, ZipDirectoryEntry $h2, $local2)
    {
    ...
    public function close()
    {
        if (!$this->_readOnly && $this->_changed) {
            ...
            foreach($this->_files as $index => $file) {
                if ($file->state == ZipDirectoryEntry::ZIP_ST_DELETED)
                    continue;
    ...
    private function zipWriteLocalEntry($fp, ZipDirectoryEntry $local, ZipDirectoryEntry $central = null)
    {
        $headerPos = ftell($fp);
        ...
        if ($local->state == ZipDirectoryEntry::ZIP_ST_UNCHANGED || $local->state == ZipDirectoryEntry::ZIP_ST_RENAMED) {
            ...
            if (strlen($this->getArchivePassword())) {
                ...
                $local->bitFlags |= ZipFile::ZIP_GPBF_ENCRYPTED;
            }
    ...
    private function zipWriteCentralDirectory($fp, ZipCentralDirectory $cd)
    {
        $size = 0;
